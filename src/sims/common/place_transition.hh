...
class SpinLock {
public:
    // No explicit init needed; std::atomic_flag is initialized using ATOMIC_FLAG_INIT.
...
{
  public:
  // explicit Place(const std::string& asid) : BasePlace(asid), tokens(100000, asid) {}
  // explicit Place(const std::string& asid, uint64_t max_size) : BasePlace(asid), tokens(max_size, asid){}

  explicit Place(const std::string& asid) : BasePlace(asid) {}
  explicit Place(const std::string& asid, uint64_t max_size) : BasePlace(asid){}

  // lpndeque<TokenType*> tokens;
  ...
  void initPlain(){
    plain = true;
    tokens.clear();
    tokens.push_back(new TokenType());
    tokens[0]->ts = tk_ts;
  }
