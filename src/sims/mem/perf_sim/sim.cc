...
void mem_advance_until_time(uint64_t ts){
    LOCK;
    uint64_t time = 0, start_time = 0, last_time = 0; 
    while(time <= ts){
    ...
    if(memory_resp_for_this_dev->tokens.size() == 0){
        ...
        *ts = (token->ts);
        ...
        if(token->type == 1){
            *buffer = 0;
            *len = 0;
            mem_write(token->pid_fd, token->addr, (void*)token->mem_buffer_ptr, (size_t)token->len);
        }else{
            mem_read(token->pid_fd, token->addr, (void*)token->mem_buffer_ptr, (size_t)token->len);
