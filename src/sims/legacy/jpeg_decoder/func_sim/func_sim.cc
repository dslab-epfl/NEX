...
#include "header/jpeg_dqt.h"
#include "header/jpeg_dht.h"
#include "header/jpeg_idct.h"
#include "header/jpeg_bit_buffer.h"
#include "header/jpeg_mcu_block.h"
...
extern "C" {
    void jpeg_lpn_driver(uint8_t* buf, uint32_t len, uint8_t* dst, uint64_t* decoded_len, int** ret_array_ptr, int* size);
}
...
void jpeg_lpn_driver(uint8_t* buf, uint32_t len, uint8_t* dst, uint64_t* decoded_len, int** ret_array_ptr, int* size) {
    ...
    dprintf("Decode done: %d\n", decode_done);
    assert(decode_done);

    if (decode_done) {
        size_t buffer_size = m_width * m_height * 2;
        uint16_t *buffer = (uint16_t *)malloc(buffer_size/4*4+4);
        *decoded_len = buffer_size;
...
#define MASK6 0b111111
    ...
    //   size_t buffer_size = header_size + m_width * m_height * 6;
    //   char *buffer = (char *)malloc(buffer_size);
    //   // Write image header to the buffer
    //   int offset = snprintf(buffer, buffer_size, "P6\n%d %d\n255\n", m_width, m_height);
    //   // Write image data to the buffer
    //   for (int y = 0; y < m_height; y++) {
    //     for (int x = 0; x < m_width; x++) {
    //       offset += snprintf(buffer + offset, buffer_size - offset, "%c%c%c",
    //                          m_output_r[(y * m_width) + x],
    //                          m_output_g[(y * m_width) + x],
    //                          m_output_b[(y * m_width) + x]);
    //     }
    //   }

      // Copy the contents into dst
      printf("Finished decode! Image size: %lu\n", buffer_size);
