...
class SRAM {
 public:
  ...
  void Load(const VTAMemInsn* op,
            ...
            bool skip_exec,
            uint64_t tag) {
    // printf("running load %p %p\n", op, load_counter);
    load_counter[0] += (op->x_size * op->y_size) * kElemBytes;
    if (skip_exec) return;
    // printf("Loading %d bytes, ysize %d, dram_base %p \n", (op->x_size * op->y_size) * kElemBytes, op->y_size, (void*)op->dram_base);
    DType* sram_ptr = data_ + op->sram_base;
    uint8_t* dram_ptr = (uint8_t*)(get_virtual(op->dram_base * (uint64_t)kElemBytes));
    ...
    // printf("id_counter: total bytes %ld, num of loads%d  \n", (op->x_size * op->y_size) * kElemBytes, op->y_size);
    for (uint32_t y = 0; y < op->y_size; ++y) {
      ...
      sram_ptr += op->x_pad_0;
      //printf("y: sram_ptr %p, dram_ptr %p \n", sram_ptr, dram_ptr);
  ...
  void Load_int8(const VTAMemInsn* op,
    ...
    if (skip_exec) return;
    DType* sram_ptr = data_ + op->sram_base;
    int8_t* dram_ptr = (int8_t*)(get_virtual(op->dram_base * kElemBytes / factor));
    uint64_t xtotal = op->x_size + op->x_pad_0 + op->x_pad_1;
    uint32_t ytotal = op->y_size + op->y_pad_0 + op->y_pad_1;
    uint64_t sram_end = op->sram_base + xtotal * ytotal;
    CHECK_LE(sram_end, kMaxNumElem);
  ...
  void TruncStore(const VTAMemInsn* op, DRAM* dram) {
    ...
    uint64_t target_width = (target_bits * kLane + 7) / 8;
    BitPacker<kBits> src(data_ + op->sram_base);
    void* req_addr = get_virtual(op->dram_base * target_width);
    BitPacker<target_bits> dst(req_addr);
    for (uint32_t y = 0; y < op->y_size; ++y) {
      for (uint32_t x = 0; x < op->x_size; ++x) {
        uint32_t sram_base = y * op->x_size + x;
        uint32_t dram_base = y * op->x_stride + x;
        id_counter++;
        enqueueReq(id_counter, (uint64_t)req_addr+dram_base*kLane*target_bits/8, kLane*target_bits/8, (int)vta::CstStr::DMA_STORE, WRITE_REQ);
...
class Device {
 public:
  ...
  int Run(vta_phy_addr_t insn_phy_addr,
    ...
    if (remain > 0) {
      id_counter++;
      enqueueReq(id_counter, (uint64_t)virtual_addr+max_insn*sizeof(VTAGenericInsn)*ites, remain * sizeof(VTAGenericInsn), (int)vta::CstStr::DMA_LOAD_INSN, READ_REQ);
    }

    VTAGenericInsn* insn = static_cast<VTAGenericInsn*>(virtual_addr);

    this->MakeInsns(insn_count, insn, ts);
    // printf("After make insn \n");
    ...
    for (uint32_t i = 0; i < insn_count; ++i) {
      // printf("Running insn %d\n", i);
      vta::Device::Run_Insn(insn+i, reinterpret_cast<void*>(this));
    }
    return 0;
  }
 ...
 private:
  ...
  void MakeInsns(int insn_count, const VTAGenericInsn* insn, uint64_t ts) {
    ...
    for (int i = 0; i < insn_count; ++i) {
      ...
      new_token->ts = ts;
      vta::pnumInsn.pushToken(new_token);
 ...
 private:
  ...
  void RunLoad(const VTAMemInsn* op) {
    ...
    if (op->memory_type == VTA_MEM_ID_INP) {
      inp_.Load(op, dram_, &(prof_->inp_load_nbytes), prof_->SkipExec(), (int)vta::CstStr::DMA_LOAD_INP);
    } else if (op->memory_type == VTA_MEM_ID_WGT) {
      wgt_.Load(op, dram_, &(prof_->wgt_load_nbytes), prof_->SkipExec(), (int)vta::CstStr::DMA_LOAD_WGT);
    } else if (op->memory_type == VTA_MEM_ID_ACC) {
      acc_.Load(op, dram_, &(prof_->acc_load_nbytes), prof_->SkipExec(), (int)vta::CstStr::DMA_LOAD_ACC);
      ...
      uop_.Load(op, dram_, &(prof_->uop_load_nbytes), false, (int)vta::CstStr::DMA_LOAD_UOP);
    } else if (op->memory_type == VTA_MEM_ID_ACC_8BIT) {
      assert(0);
      acc_.Load_int8(op, dram_, &(prof_->acc_load_nbytes), prof_->SkipExec(), (int)vta::CstStr::DMA_LOAD_ACC);
    } else {
...
void AccVMVTADeviceFree(VTADeviceHandle handle) {
  id_counter = 0;
  delete static_cast<vta::Device*>(handle);
}

int AccVMVTADeviceRun(VTADeviceHandle handle,
                 vta_phy_addr_t insn_phy_addr,
                 uint32_t insn_count,
                 uint32_t wait_cycles, 
                 uint64_t ts) {
  return static_cast<vta::Device*>(handle)->Run(
      insn_phy_addr, insn_count, wait_cycles, ts);
}