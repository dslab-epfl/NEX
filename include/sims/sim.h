...
#define EXTERN_LPN_FUNC(name) \
  ...
  extern void vta_sim_start(uint64_t insn_phy_addr, uint32_t insn_count, uint32_t wait_cycles, uint64_t ts);
...
void advance_until_time(int type, uint64_t ts);
  ...
  extern void protoacc_sim_start(int pid, uint64_t msg_start_addr, uint64_t serialized_msg_addr, uint64_t descriptor_table_addr, uint64_t src_base_addr, uint64_t ts);
#endif